function shouldMerge = decideMerge_specificity(pc_scores_cluster1, pc_scores_cluster2, ...
...
% if acg number is high, we merge if passing acg check or pass a hard bimodal thres
    ...
    X = [pc_scores_cluster1; pc_scores_cluster2];
    y = [ones(size(pc_scores_cluster1, 1), 1); 2 * ones(size(pc_scores_cluster2, 1), 1)];
    ...
    if (sum(ACG) < acg_minimum_point) || isempty(ACG(refrctory_window_indices))
        ...
        if (specificity_score < specificity_thres)
            shouldMerge = true;
        else
            shouldMerge = false;
        end
